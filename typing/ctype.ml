...
let newvar ?name ?(layout=Layout.value) () =
  newty2 !current_level (Tvar {name; layout})
let newvar2 ?name ~layout level = newty2 level (Tvar {name; layout})
let new_global_var ?name layout = newty2 !global_level (Tvar {name; layout})
...
let rec copy ?partial ?keep_names scope ty =
  ...
  | _ ->
    ...
    newty2 forget (Tvar {name = None; layout = Layout.any })
    else
    let desc = ty.desc in
                ...
                match partial with
                  Some (free_univars, false) ->
                    let more' =
                      ...
                      newty2 lv (Tvar { name = None; layout = Layout.any })
                    in
                    let not_reither (_, f) =
                      match row_field_repr f with
                        Reither _ -> false
...
let existential_name cstr ty = match repr ty with
  | {desc = Tvar {name = Some name; _}} -> "$" ^ cstr.cstr_name ^ "_'" ^ name
  | _ -> "$" ^ cstr.cstr_name
  ...
  match ty with
    {desc = Tconstr (path, args, _abbrev); level = level} ->
      begin try
        ...
        (* FIXME_layout: what does this do? *)
        ignore
          (subst env level Public (ref Mnil) None decl.type_params args
             (newvar2 ~layout:Layout.any level))
      with Not_found -> ()
      end
  | _ ->
      assert false
...
let rec cmp_layout env may_refine_var may_refine_env ty layout =
  ...
  | Tconstr (path, tl, _abbrev) ->
    ...
    | exception Not_found when Layout.subset layout Layout.value ->
      let rec str_of_path : Path.t -> string = function
        | Pident i -> Ident.name i
        | Pdot (t, s) -> str_of_path t ^ "." ^ s
        | Papply (f, _) -> str_of_path f ^ "(..)" in
      let s = Printf.sprintf "%s: %s\n" (Sys.getcwd ()) (str_of_path path) in
      (match str_of_path path with "unit" | "list" | "int" | "string" | "option" -> failwith "wtf?" | _ -> ());
      let fd = open_out_gen [Open_append; Open_creat] 0o666 "/tmp/missing.txt" in
      output_string fd s;
      Printexc.get_callstack 10000 |> Printexc.print_raw_backtrace fd;
      close_out fd;
      ()  (* FIXME_layout: unsound hack *)
    | exception Not_found ->
  ...
  | Tfield _ | Tnil ->
    check_sub Layout.value (* FIXME_layout *)
...
and make_rowvar level use1 rest1 use2 rest2  =
  let set_name ty name =
    match ty.desc with
      Tvar ({ name = None; _ } as n) ->
       set_type_desc ty (Tvar {n with name})
    | _ -> ()
  in
  let name =
  ...
  if use2 then rest2 else newvar2 ?name ~layout:Layout.value level
...
let filter_arrow env t l =
  ...
  match t.desc with
    Tvar _ ->
      let lv = t.level in
      let t1 = newvar2 ~layout:Layout.any lv
      and t2 = newvar2 ~layout:Layout.any lv in
...
let rec filter_method_field env name priv ty =
  ...
  match ty.desc with
    Tvar _ ->
      ...
      let ty1 = newvar2 ~layout:Layout.value level
      and ty2 = newvar2 ~layout:Layout.value level in
...
let matches env ty ty' =
  ...
  cleanup_abbrev ();
  let ok =
    (* FIXME_layout: check that layouts have not been refined *)
    ...
    && not (has_constr_row' env t) ->
      ...
      begin try match t'.desc with
        Tobject _ when posi && not (opened_object t') ->
          ...
          ty.desc <- Tvar {name = None; layout = Layout.value };
          let t'' = newvar () in
...
let rec unalias_object ty =
  ...
  | Tunivar _ ->
      ty
  | Tconstr _ ->
      (*FIXME_layout constructor layouts. Also wtf. *)
      newvar2 ~layout:Layout.value ty.level
  | _ ->
      assert false
...
let rec nondep_type_rec ?(expand_private=false) env ids ty =
  ...
  with Not_found ->
    let ty' = newgenvar Layout.any in        (* Stub *)
