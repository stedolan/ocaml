...
let mode_subcomponent expected_mode =
  let mode = Value_mode.nonregional expected_mode.mode in
  let position = Nontail in
  let escaping_context = None in
  { position; escaping_context; mode }
  ...
  | Ppat_record(lid_sp_list, closed) ->
      ...
      let type_label_pat (label_lid, label, sarg) k =
        ...
        let alloc_mode =
          ...
          | Nonlocal -> Value_mode.nonlocal alloc_mode
...
and type_expect_
  ...
  | Pexp_record(lid_sexp_list, opt_sexp) ->
      ...
      let rmode = Value_mode.nonregional expected_mode.mode in
      let lbl_exp_list =
  ...
  | Pexp_field(srecord, lid) ->
      ...
      let mode =
        match label.lbl_global with
        | Global -> Value_mode.global
        | Nonlocal -> Value_mode.nonlocal rmode
        | Unrestricted -> rmode
      in
  ...
  | Pexp_lazy e ->
      ...
      with_explanation (fun () ->
        unify_exp_types loc env to_unify (generic_instance ty_expected));
      let env = Env.add_lock alloc_mode env in
...
let escaping_hint reason context =
  ...
  | Types.Value_mode.Locality, Some Tailcall_argument ->
      [ Location.msg
          "@[Hint: Local values cannot be arguments to tail-calls@ \
           unless the result of the tail-call has a \"local_\" annotation@]" ]
  | Types.Value_mode.Locality, Some Tailcall_function ->
      [ Location.msg
          "@[Hint: Local values cannot be called in tail position@ \
           unless the result of the call has a \"local_\" annotation@]" ]
