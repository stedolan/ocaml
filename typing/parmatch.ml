...
let extra_pat =
  make_pat
    ...
    Ctype.none Value_mode.min_mode Env.empty
...
let mark_partial =
  let zero =
    make_pat (`Constant (Const_int 0)) Ctype.none Value_mode.min_mode Env.empty
  in
...
let ppat_of_type env ty =
  match pats_of_type env ty Value_mode.min_mode with
  | [] -> PT_empty
