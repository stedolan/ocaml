...
(** [Memprof] is a sampling engine for allocated memory words. Every
   ...
   words, a block can potentially be sampled several times. Once a
   block is sampled, it becomes tracked. A tracked block triggers a
...
module Memprof :
  sig
    ...
    (** The type of metadata associated with allocations. This is the
       ...
       sampling of a minor allocation. *)
    ...
    (** Start the sampling with the given parameters. If another
       ...
       in which case reentrancy has to be taken into account.

       Note that the callback can be postponed slightly after the
       actual event. Therefore, the context of the callback may be
       slightly different than expected.

       Calling [start] or [stop] in a callback can lead to callbacks
       not being called even though some event happened. *)
