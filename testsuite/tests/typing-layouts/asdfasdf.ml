module Monad_intf = struct
  ...
  (* CR-someday vtjeng: Since Monad subsumes Applicative, consider making Monad.S a superset
     of Applicative.S (and the same for Monad.S2 and Applicative.S2). This would also mean
     we can standardize the implementation of [all] with applicative (or simply rely on the
     implementation found in applicative). *)
  ...
  module Make_general (M : Basic_general) = struct
    ...
    module Let_syntax = struct
      ...
      module Let_syntax = struct
        ...
        let bind = bind
        let map = map
        let both a b = assert false

        module Open_on_rhs = struct end
      end
    end

    let join t = assert false
