...
CAMLprim value caml_ml_output_bigarray(value vchannel, value arr,
                                       value vpos, value vlen)
{
  CAMLparam4(vchannel, arr, vpos, vlen);
  struct channel * channel = Channel(vchannel);
  intnat pos = Long_val(vpos);
  intnat len = Long_val(vlen);

  Lock(channel);
  caml_really_putblock(channel, Caml_ba_data_val(arr) + pos, len);
  Unlock(channel);
...
CAMLprim value caml_ml_input_bigarray(value vchannel, value arr,
                                      value vpos, value vlen)
{
  CAMLparam4(vchannel, arr, vpos, vlen);
  struct channel * channel = Channel(vchannel);
  ...
  Lock(channel);
  n = caml_getblock(channel, Caml_ba_data_val(arr) + pos, len);
  Unlock(channel);
