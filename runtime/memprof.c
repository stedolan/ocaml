 ...
 *   entries, so they are simply deleted. The new profile effectively
 *   begins with the following allocation.
 *
                              ...
                              bool *moved_p)
{
  ...
  moved = (es != new_es);
  es = new_es;
  if (!es) { /* This entry was evicted while we were running. */
    res = Val_unit;
    goto done;
  }
  ...
  if (es && validated_config(es) == CONFIG_NONE) {
  ...
  if (Is_exception_result(res) || res == Val_unit) {
    ...
    /* The callback we just ran was not a dealloc, (because they
     * return unit) so (if the block has since been deallocated) there
     * may be more callbacks to run on this entry. */
                              ...
                              int allocs, unsigned char* encoded_lens)
{
   ...
   * callbacks up to the one which started the new profile.
   *
   ...
   * deleted (or marked as deallocated if the allocation callback
  ...
  if (!restarted) {
     ...
     * domain. */
    entries_transfer(entries, &domain->entries);
  }
