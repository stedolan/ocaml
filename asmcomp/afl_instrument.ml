...
let rec with_afl_logging b =
  if afl_inst_ratio < 100 &&
    Random.int 100 >= afl_inst_ratio then instrument b else
  let instrumentation =
    ...
    Clet(cur_pos,  op Cxor [op (Cload Word_int)
...
and instrument = function
  ...
  | Ccatch (case, ids, e1, e2) ->
     Ccatch (case, ids, instrument e1, instrument e2)
...
let instrument_initialiser c =
  ...
  with_afl_logging
    (Csequence
       (Cop (Cextcall ("caml_setup_afl", typ_int, false, Debuginfo.none, None),
             [Cconst_int 0]
             ),
        c))
